...
The team's system leans on PhenoCam imagery for its visual read. The background for this project is the other half of that same question — an in-situ sensor pipeline was already run (CoAgMET/RAWS/SNOTEL/Zentra soil moisture across Colorado) and do SSURGO-based soil modeling with satellite land/soil products via Google Earth Engine. So off the back of the team's CLBJ work, a narrower question was asked: before a satellite-derived signal is trusted as a stand-in for ground sensors, does it actually track them — and where does it not?
